...
static bool do_predset(DisasContext *s, int esz, int rd, int pat, bool setflag)
{
    ...
    for (i = 0; i < ALIGN_DOWN(setsz, 8); i += 8) {
...
TRANS_FEAT(ZIP2_q, aa64_sve_f64mm, gen_gvec_ool_arg_zzz,
           ...
           ALIGN_DOWN(vec_full_reg_size(s), 32) / 2)
...
static bool trans_WHILE_ptr(DisasContext *s, arg_WHILE_ptr *a)
{
    ...
    if (a->rw) {
        ...
        tcg_gen_andi_i64(diff, diff, ~0ull << a->esz);
...
// TODO: Add all float{16,32,64}_{zero,half,one,two} and restore.
...
{
    int len_align = ALIGN_DOWN(len, 8);
                          ...
                          const GVecGen2i ops[3])
{
    ...
    tlib_assert(a->imm > 0 && a->imm <= (8 << a->esz));
    if (sve_access_check(s)) {
        unsigned vsz = vec_full_reg_size(s);
        tcg_gen_gvec_2i(vec_full_reg_offset(s, a->rd),
                        vec_full_reg_offset(s, a->rn),
