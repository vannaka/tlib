...
void glue(helper_vfwredosum_vs, POSTFIX)(CPUState *env, uint32_t vd, uint32_t vs2, uint32_t vs1)
{
    ...
    case 32:
        ...
        acc = helper_fcvt_s_d(env, ((uint32_t *)V(vs1))[0], env->frm);
        break;
    default:
        helper_raise_exception(env, RISCV_EXCP_ILLEGAL_INST);
        return;
    ...
    for (int ei = 0; ei < env->vl; ++ei) {
        ...
        switch (eew) {
        case 32:
            acc = helper_fadd_d(env, acc, helper_fcvt_s_d(env, ((uint32_t *)V(vs2))[ei], env->frm), env->frm);
            break;
        }
    }
    switch (eew) {
    case 32:
        ((uint64_t *)V(vd))[0] = helper_fcvt_s_d(env, ((uint32_t *)V(vs1))[0], env->frm);
        break;
    }
}

void glue(helper_vfrsqrt7_v, POSTFIX)(CPUState *env, uint32_t vd, uint32_t vs2)
{
    require_fp;
    if (V_IDX_INVALID(vd) || V_IDX_INVALID(vs2)) {
        helper_raise_exception(env, RISCV_EXCP_ILLEGAL_INST);
    ...
    for (int ei = 0; ei < env->vl; ++ei) {
        ...
        switch (eew) {
        case 32:
            ((uint32_t *)V(vd))[ei] = f32_rsqrte7(env, ((uint32_t *)V(vs2))[ei]);
            break;
        case 64:
            ((uint64_t *)V(vd))[ei] = f64_rsqrte7(env, ((uint64_t *)V(vs2))[ei]);
            break;
        }
    }
}

void glue(helper_vfrec7_v, POSTFIX)(CPUState *env, uint32_t vd, uint32_t vs2)
{
    require_fp;
    if (V_IDX_INVALID(vd) || V_IDX_INVALID(vs2)) {
        helper_raise_exception(env, RISCV_EXCP_ILLEGAL_INST);
    ...
    for (int ei = 0; ei < env->vl; ++ei) {
        ...
        switch (eew) {
        case 32:
            ((uint32_t *)V(vd))[ei] = f32_recip7(env, ((uint32_t *)V(vs2))[ei]);
            break;
        case 64:
            ((uint64_t *)V(vd))[ei] = f64_recip7(env, ((uint64_t *)V(vs2))[ei]);
            break;
        }
    }
}
