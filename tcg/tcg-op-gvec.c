...
// First include is for 'helper_*' prototypes and the second is for 'gen_helper_*'.
...
#define tcg_constant_ptr tcg_const_ptr
                   ...
                   uint64_t in_c)
{
    ...
    assert(vece <= (in_32 != TCGv_NULL ? MO_32 : MO_64));
    assert(in_32 == TCGv_NULL || in_64 == TCGv_NULL);

    /* If we're storing 0, expand oprsz to maxsz.  */
    if (in_32 == TCGv_NULL && in_64 == TCGv_NULL) {
    ...
    type = choose_vector_type(NULL, vece, oprsz,
                              (TCG_TARGET_REG_BITS == 64 && in_32 == TCGv_NULL
                               && (in_64 == TCGv_NULL || vece == MO_64)));
    if (type != 0) {
        TCGv_vec t_vec = tcg_temp_new_vec(type);

        if (in_32 != TCGv_NULL) {
            tcg_gen_dup_i32_vec(vece, t_vec, in_32);
        } else if (in_64 != TCGv_NULL) {
...
static void expand_clr(uint32_t dofs, uint32_t maxsz)
{
    do_dup(MO_8, dofs, maxsz, maxsz, TCGv_NULL, TCGv_NULL, 0);
}
                          ...
                          uint32_t maxsz, TCGv_i32 in)
{
    ...
    do_dup(vece, dofs, oprsz, maxsz, in, TCGv_NULL, 0);
}
                          ...
                          uint32_t maxsz, TCGv_i64 in)
{
    ...
    do_dup(vece, dofs, oprsz, maxsz, TCGv_NULL, in, 0);
}
...
{
    check_size_align(oprsz, maxsz, dofs);
    do_dup(vece, dofs, oprsz, maxsz, TCGv_NULL, TCGv_NULL, x);
}
                      ...
                      uint32_t oprsz, uint32_t maxsz)
{
    ...
    if (cond == TCG_COND_NEVER || cond == TCG_COND_ALWAYS) {
        do_dup(MO_8, dofs, oprsz, maxsz,
               TCGv_NULL, TCGv_NULL, -(cond == TCG_COND_ALWAYS));
        return;
    }
